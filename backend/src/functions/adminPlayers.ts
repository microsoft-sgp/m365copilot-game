...
async function getPlayerDetail(request: HttpRequest, _context: InvocationContext) {
  ...
  const sessions = await pool.request().input('playerId', sql.Int, id).query(`
  ...
  const submissions = await pool.request().input('playerId', sql.Int, id).query(`
...
async function deletePlayer(request: HttpRequest, _context: InvocationContext) {
  ...
  await pool
    ...
    .query('DELETE FROM game_sessions WHERE player_id = @playerId;');
  await pool
    .request()
    .input('playerId', sql.Int, id)
    .query('DELETE FROM submissions WHERE player_id = @playerId;');
  await pool.request().input('id', sql.Int, id).query('DELETE FROM players WHERE id = @id;');

  return { jsonBody: { ok: true } };
}

async function revokeSubmission(request: HttpRequest, context: InvocationContext) {
  const auth = verifyAdmin(request);
  if (!auth.ok) return auth.response;
  ...
  const result = await pool
    .request()
    .input('id', sql.Int, id)
    .query('DELETE FROM submissions OUTPUT deleted.campaign_id WHERE id = @id;');
  ...
  await invalidateLeaderboardCache(
    result.recordset[0]?.campaign_id || getDefaultCampaignId(),
    context,
  );
...
app.http('adminSearchPlayers', {
  methods: ['GET'],
  authLevel: 'anonymous',
  route: 'portal-api/players',
  handler: searchPlayers,
});
app.http('adminGetPlayerDetail', {
  methods: ['GET'],
  authLevel: 'anonymous',
  route: 'portal-api/players/{id}',
  handler: getPlayerDetail,
});
app.http('adminDeletePlayer', {
  methods: ['DELETE'],
  authLevel: 'anonymous',
  route: 'portal-api/players/{id}',
  handler: deletePlayer,
});
app.http('adminRevokeSubmission', {
  methods: ['DELETE'],
  authLevel: 'anonymous',
  route: 'portal-api/submissions/{id}',
  handler: revokeSubmission,
});

export { searchPlayers, getPlayerDetail, deletePlayer, revokeSubmission };
