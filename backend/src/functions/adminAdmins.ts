...
import { ADMIN_COOKIE_NAMES, getCookie } from '../lib/adminCookies.js';
...
function requireStepUp(
  ...
  const proof = verifyAdminStepUpToken(
    getCookie(request, ADMIN_COOKIE_NAMES.stepUp),
    actingEmail || '',
    { action, targetEmail },
  );
...
async function addAdmin(request: HttpRequest, context: InvocationContext) {
  ...
  const stepUpFailure = requireStepUp(request, auth.email, 'add-admin', email);
  ...
  await pool
    ...
    .input('createdBy', sql.NVarChar(320), auth.email).query(`
...
async function removeAdmin(request: HttpRequest, context: InvocationContext) {
  ...
  const stepUpFailure = requireStepUp(request, auth.email, 'remove-admin', email);
  if (stepUpFailure) return stepUpFailure;
  ...
  if (getAdminEmails().includes(email)) {
    return {
      ...
      jsonBody: {
        ok: false,
        message: 'Bootstrap admins are managed through Function App settings',
      },
    };
  }
  ...
  if (
    ...
    activeDbAdmins.length <= 1 &&
    activeDbAdmins.includes(email)
  ) {
    return {
      status: 409,
  ...
  const result = await pool
    .request()
    .input('email', sql.NVarChar(320), email)
    .input('disabledBy', sql.NVarChar(320), auth.email).query(`
      UPDATE admin_users
      SET is_active = 0,
...
app.http('adminListAdmins', {
  methods: ['GET'],
  authLevel: 'anonymous',
  route: 'portal-api/admins',
  handler: listAdmins,
});
app.http('adminAddAdmin', {
  methods: ['POST'],
  authLevel: 'anonymous',
  route: 'portal-api/admins',
  handler: addAdmin,
});
app.http('adminRemoveAdmin', {
  methods: ['DELETE'],
  authLevel: 'anonymous',
  route: 'portal-api/admins/{email}',
  handler: removeAdmin,
});
