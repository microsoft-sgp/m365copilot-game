...
async function listOrganizations(request, context) {
...
async function updateOrganization(request, context) {
  const auth = verifyAdmin(request);
  if (!auth.ok) return auth.response;
...
async function deleteOrganization(request, context) {
  const auth = verifyAdmin(request);
  if (!auth.ok) return auth.response;
  ...
  if (subs.recordset[0].cnt > 0) {
    return { status: 409, jsonBody: { ok: false, message: 'Cannot delete organization with existing submissions' } };
  }
...
async function addDomain(request, context) {
  const auth = verifyAdmin(request);
  if (!auth.ok) return auth.response;
...
async function removeDomain(request, context) {
  const auth = verifyAdmin(request);
  if (!auth.ok) return auth.response;
