...
describe('adminPlayers.searchPlayers', () => {
  it('wraps the query parameter with LIKE wildcards', async () => {
    ...
    const res = await searchPlayers(fakeRequest({ query: { q: 'ali' }, headers }), {});
...
describe('adminPlayers.getPlayerDetail', () => {
  it('rejects non-numeric id', async () => {
    mockPool = createMockPool([]);
    const res = await getPlayerDetail(fakeRequest({ params: { id: 'abc' }, headers }), {});
    expect(res.status).toBe(400);
  });

  it('returns 404 when player does not exist', async () => {
    mockPool = createMockPool([[]]);
    const res = await getPlayerDetail(fakeRequest({ params: { id: '5' }, headers }), {});
...
describe('adminPlayers.deletePlayer', () => {
  it('rejects non-numeric id', async () => {
    mockPool = createMockPool([]);
    const res = await deletePlayer(fakeRequest({ params: { id: 'abc' }, headers }), {});
    expect(res.status).toBe(400);
  });
  ...
  it('deletes in FK order: tile_events, game_sessions, submissions, players', async () => {
    ...
    const res = await deletePlayer(fakeRequest({ params: { id: '5' }, headers }), {});
...
describe('adminPlayers.revokeSubmission', () => {
  it('rejects invalid id', async () => {
    mockPool = createMockPool([]);
    const res = await revokeSubmission(fakeRequest({ params: { id: 'nope' }, headers }), {});
    expect(res.status).toBe(400);
  });

  it('returns 404 when nothing deleted', async () => {
    mockPool = createMockPool([{ recordset: [], rowsAffected: [0] }]);
    const res = await revokeSubmission(fakeRequest({ params: { id: '7' }, headers }), {});
    expect(res.status).toBe(404);
  });
