...
- **THEN** the system MUST fetch data from `GET /api/admin/dashboard` and display summary cards for total players, total sessions, total submissions, average tiles cleared, and top organization
...
- **THEN** the system MUST call `GET /api/admin/export` with the admin JWT and trigger a file download of the CSV
...
- **THEN** the system MUST call `GET /api/admin/players?q=<query>` and display matching players with their name, email, session count, and submission count
...
- **THEN** the system MUST call `GET /api/admin/players/:id` and display the player's full detail including all sessions with progress and all submissions with keywords

### Requirement: Admin player search endpoint
The system SHALL expose `GET /api/admin/players` to search players by email or name, protected by admin authentication.

#### Scenario: Search players by query
- **GIVEN** the admin provides a query parameter `q`
- **WHEN** `GET /api/admin/players?q=alice` is called
...
- **THEN** the system MUST call `DELETE /api/admin/submissions/:id` and remove the submission from the database, updating the leaderboard accordingly
...
- **THEN** the system MUST call `POST /api/admin/campaigns/:id/clear` which deletes all tile_events, game_sessions, and submissions for that campaign (preserving players and organizations)
...
- **THEN** the system MUST call `POST /api/admin/campaigns/:id/reset-leaderboard` which deletes all submissions for that campaign (preserving sessions and events)
