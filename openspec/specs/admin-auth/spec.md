...
Defines admin authentication via OTP-based login flow and JWT token management for securing admin API endpoints.
...
The system SHALL expose `POST /api/portal-api/verify-otp` to validate a 6-digit OTP and return a signed JWT token on success.
...
- **THEN** the system MUST call `POST /api/portal-api/verify-otp`, store the returned JWT in `sessionStorage`, and navigate to the admin dashboard view
