...
export function buildMarker(packId, tileIndex) {
...
export function buildPromptFull(promptTemplate, packId, tileIndex) {
  const concrete = buildMarker(packId, tileIndex);
  return promptTemplate.replace(
    /VERIFY-[A-Z0-9]+-PACK-TILE/g,
    concrete,
  );
}
...
export function verifyStructure(proof, packId, tileIndex, rules) {
  const errors = [];
  ...
  if (rules.tableRows) {
    const rows = (proof.match(/\|.*\|/g) || []).filter(
      (r) => !/^[\s|:-]+$/.test(r),
    );
    if (rows.length < rules.tableRows) {
      errors.push(
        `Table needs at least ${rules.tableRows} data rows (found ${rows.length})`,
      );
    }
  }
  if (rules.tableCols) {
    ...
    if (rows.length > 0) {
      ...
      if (cols < rules.tableCols) {
        errors.push(
          `Table needs EXACTLY ${rules.tableCols} columns (found ~${cols})`,
        );
      }
    }
  }
  ...
  if (rules.qaCount) {
    ...
    if (qs < rules.qaCount) {
      errors.push(
        `Need EXACTLY ${rules.qaCount} Q&A pairs (Q: lines found: ${qs})`,
      );
    }
  }
  if (rules.hashtags) {
  ...
  if (rules.sentences) {
    ...
    if (sents.length < rules.sentences) {
      errors.push(
        `Need EXACTLY ${rules.sentences} sentences (found ${sents.length})`,
      );
    }
  }
  if (rules.subBullets) {
...
export function validateKeywordFormat(kw) {
