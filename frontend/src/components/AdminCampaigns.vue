...
<template>
  <div>
    ...
    <!-- Create form -->
    <div v-if="showCreate" class="glass mb-5 rounded-xl p-4">
      <div class="mb-2 grid grid-cols-2 gap-3">
        <div>
          <label class="field-label">Campaign ID</label>
          <input v-model="newCampaign.id" class="field-input" placeholder="e.g. JUL26" />
        </div>
        <div>
        ...
        <div>
          ...
          <input v-model.number="newCampaign.totalPacks" class="field-input" type="number" />
        </div>
        <div>
          <label class="field-label">Total Weeks</label>
          <input v-model.number="newCampaign.totalWeeks" class="field-input" type="number" />
        </div>
      </div>
      <div class="mb-3">
        <label class="field-label">Copilot URL</label>
        <input v-model="newCampaign.copilotUrl" class="field-input" />
      </div>
      ...
      <div v-for="c in campaigns" :key="c.id" class="glass mb-3 rounded-xl p-4">
        ...
        <template v-if="editing && editing.id === c.id">
          <div class="mb-2 grid grid-cols-2 gap-3 text-label-lg">
            ...
            <div>
              <label class="field-label">Total Packs</label>
              <input v-model.number="editing.totalPacks" class="field-input" type="number" />
            </div>
            <div>
              <label class="field-label">Total Weeks</label>
              <input v-model.number="editing.totalWeeks" class="field-input" type="number" />
            </div>
            <div>
              <label class="field-label">Copilot URL</label>
              <input v-model="editing.copilotUrl" class="field-input" />
            </div>
          </div>
        ...
        <template v-else>
          <div class="mb-1 flex items-center justify-between">
            ...
            <div class="flex gap-1">
              ...
              <button class="btn btn-ghost btn-xs" @click="toggleActive(c)">
